...
describe('buildDomiciliosDocHtml', () => {
  ...
  test('the only external URLs are Google Maps links', () => {
    // Includes a household with coordinates so this test actually exercises
    // an emitted https:// link, not just the absence of one.
    const comCoord = [...domicilios, { endereco: 'RUA Y, Nº 10', nDomicilio: '3', lat: -12.9, lon: -38.5, agendado: '', futura: false, situacao: '', transmissao: '' }];
    const html = buildDomiciliosDocHtml(meta, resumoHtml, comCoord);
