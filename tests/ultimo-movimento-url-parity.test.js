...
// Two independent files each fetch the Último Movimento report and
...
// since retired — its capability now lives in the Último Movimento
// "Mapa" panel) went unnoticed and unfixed when the first two were
// patched for this same endpoint migration. This file is the guard
// against that recurring: it pins both remaining builders to produce
// byte-identical URLs for the same inputs, so a future edit to one that
// silently diverges from the other fails a test instead of shipping
// quietly broken.
...
describe('Último Movimento URL parity across the two independent builders', () => {
  test('simple mode, no F5 gateway: both agree', () => {
  ...
  test('simple mode, F5 gateway: both agree', () => {
  ...
  test('fallback mode, F5 gateway (doubled /relatorio segment): both agree', () => {
